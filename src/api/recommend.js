/*
 * @Author: 少华
 * @Date: 2020-01-14 19:55:54
 * @LastEditors: 少华
 * @LastEditTime: 2020-01-14 19:57:01
 * @Description: update time
 */
import {commonParams,API_IP,API_PORT} from './config'
import axios from 'axios'

// const debug = process.env.NODE_ENV !== 'production'
export function getRecommend() {
  const url = API_IP+':'+ API_PORT +'/api/getTopBanner'
...
export function getDiscList () {
  const url = API_IP+':'+ API_PORT +'/api/getDiscList'
...
export function getSongList (disstid) {
  const url = API_IP+':'+ API_PORT +'/api/getCdInfo'
